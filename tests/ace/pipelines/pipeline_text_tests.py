...
class PipelineTextTest(unittest.TestCase):
    ...
    def test_features_dims(self):
        ...
        pipe_text.fit([self.__text['text1']], self.__text['label'])
        X=pipe_text.transform([self.__text['text1']], self.__text['label'])
        n_rows = X.shape[0]
        n_cols=X.shape[1]

        self.assertEqual(n_rows, 10)
        self.assertEqual(n_cols, 29)
